...
model_names = ['msdnet', 'mynet', 'lstm', 'dcl']

arg_parser = argparse.ArgumentParser(
                description='Image classification PK main script')
# 添加实验设置参数组exp_group
exp_group = arg_parser.add_argument_group('exp', 'experiment setting')
exp_group.add_argument('--save', default='save/Myresnet/UniMib_v1',
                       type=str, metavar='SAVE',
                       help='path to the experiment logging directory ')
exp_group.add_argument('--resume', default=None, type=str, help='Name of latest checkpoint (default: none)')
exp_group.add_argument('--evalmode', default='dynamic', choices=['anytime', 'dynamic'], help='which mode to evaluate')
exp_group.add_argument('--evaluate-from', default=r'save\Myresnet\UniMib_v1\save_models\model_best_acc.pth.tar', type=str, metavar='PATH', help='path to saved checkpoint (default: none)')
...
arch_group.add_argument('--arch', '-a', metavar='ARCH', default='mynet',
...
arch_group.add_argument('--nChannels', type=int, default=64)# 设置块的通道数 # dcl: UCI 32, other 64
...
arg_parser.add_argument('--MIE', action='store_true', default=True, help='Use model-internal ensembling')# 是否在模型内部使用集成学习

arg_parser.add_argument('--optimize_temperature', action='store_true', default=True, help='Use the validation set to optimize temperature scaling individually for each block')# 是否使用验证集优化温度衰减参数（超参数）
