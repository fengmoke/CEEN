...
def dynamic_evaluate(model, test_loader, val_loader, args, prints = False):
    ...
    if args.arch not in ['dcl', 'lstm']:
        np.save(fr'plot\data_result\{dataset_name}\correct_label{fname_ending}.npy', val_target.numpy())
        np.save(fr'plot\data_result\{dataset_name}\logit{fname_ending}.npy', val_pred.numpy())
        np.save(fr'plot\data_result\{dataset_name}\val_label{fname_ending}.npy', argmax_val.numpy())
    # print(val_pred, val_pred.shape, type(val_pred))
    # print(type(argmax_val))
