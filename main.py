...
def train(train_loader, model, criterion, optimizer, epoch):# 训练函数
    global args
...
def validate(val_loader, model, criterion):# 验证函数
    ...
    with torch.no_grad():
        ...
        np.savetxt(r'plot\data_result\error_uncertainty.txt', entropy)
        np.save(r'plot\data_result\softmax_value.npy', softmax_value_numpy)
        np.savetxt(r'plot\data_result\correct_label.txt', correct_label_numpy)
        # print(entropy.shape)
